  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("http://localhost:3000/api/consultation?bookingType=formalDiagnosis", {
        ...
        body: JSON.stringify({ email, date, timeSlot, bookingType: "formalDiagnosis" }), // Include all required fields
      });
    ...
    <div className="flex flex-col items-center px-6 min-h-screen pb-6 pt-6 h-full">
      ...
      <div className="main w-full max-w-screen-lg flex flex-col md:flex-row justify-evenly flex-wrap py-4 mt-7 gap-4">
        <div className="flex-1 min-w-[250px]">
          ...
          <BookingSchedule setDate={setDate} setTimeSlot={setTimeSlot} bookingType="formalDiagnosis" />
        </div>
      </div>
      <div className="mt-10">
        <button
