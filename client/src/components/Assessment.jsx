...
const Assessment = () => {
  ...
  const submitAssessment = (token) => {
    fetch(`https://leeza.app/api/assessment?test=${newid}`, {
      ...
      .then(data => {
        console.log('Assessment result data:', data); // Log the data to see the response
        setResult(data.prediction);
  ...
  const answeredQuestions = responses.filter(response => response !== '').length;
    ...
    <div className="App p-8 max-w-3xl mx-auto bg-white shadow-md rounded">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {startIndex + QUESTIONS_PER_PAGE >= questions.length && (
            <div className="flex justify-center mt-4">
              <button
                type="submit"
                className="bg-purple-400  text-white font-bold py-3 px-6 rounded-lg mt-4 text-lg md:text-xl w-full md:w-auto max-w-xs md:max-w-md shadow-lg transition duration-300 ease-in-out transform hover:scale-105"
              >
                Submit
              </button>
            </div>
      ...
      <div className="mt-8">
        ...
        <p className="text-center text-lg font-medium">
          {answeredQuestions} out of {totalQuestions} questions attempted
        </p>
      </div>
